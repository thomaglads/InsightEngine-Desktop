...
export const FileUploader = ({ onFileUpload, loading, disabled }) => {
  ...
  const handleFiles = async (files) => {
    ...
    try {
      ...
      const analysis = CSVAnalyzer.analyze(content);
      onFileUpload(file, content, analysis);
  ...
  return (
    <div className="p-6">
      <label className={`group flex flex-col items-center justify-center h-40 border-2 border-dashed rounded-xl cursor-pointer transition-all ${
        dragActive 
        ...
        <Folder 
          ...
          className={`mb-3 transition-colors ${
            dragActive ? 'text-yellow-400' : 'text-yellow-500 group-hover:text-yellow-400'
