...
export class DatabaseIntrospection {
  ...
  inferTypeFromSample(sampleValues) {
    ...
    // Test for numeric (Safely handle BigInt)
  ...
  generateRecommendations(columns, sampleData) {
    ...
    highMissingColumns.forEach(col => {
      const values = sampleData.map(row => row[col.name]);
      const nonNullValues = values.filter(val => val !== null && val !== undefined && val !== '');
      const missingPct = Math.round(((values.length - nonNullValues.length) / values.length) * 100);

      recommendations.push({
        type: 'data_quality',
        priority: 'medium',
        message: `Consider data cleaning for ${col.name} (${missingPct}% missing)`
      });
    });
